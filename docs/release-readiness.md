...
1. Vercel Productionの対象commit、HTTPSドメイン、公開環境変数名、Production build、主要routeの実データまたは日本語エラーを確認する。追跡`vercel.json`はなく、プロジェクト設定は外部管理です。今回の管理側確認では、コードから参照されていない`SUPABASE_SECRET_KEY`がProduction/Preview環境に残存しています。値は表示・記録せず、所有者承認後の削除を推奨します（この監査では外部状態を変更しません）。
2. Supabase本番のmigration履歴、PostGIS/RLS/RPC、バックアップと空き容量を確認する。テストから本番DBへ接続しない。
3. `update-weather` Edge Functionのrevision、専用Bearer secret、Vault/Cron、boundedな一地点一日のJMA smoke、24正時・平均・冪等性を確認する。JMAの可用性は保証されないため、手動CSVの代替手順も準備する。
4. Google OAuthのGoogle側client/redirect URIとSupabase provider、Site URL・callback allow-listを設定して専用アカウントで確認する。Google OAuthは未設定なら受入完了にしない。
5. 本番SMTPの送信元・認証・レート制限と確認メール／パスワード再設定を確認する。
6. iPhone Safari/PWAとAndroid Chrome/PWAで、インストール、主要登録、地図、通信断からの復帰、屋外視認性を実機確認する。PlaywrightのPixel 7相当エミュレーションは実機ゲートの代わりにしない。
7. 2026年MAFF久井町の監査台帳、2,010件、`ST_IsValid`/修復件数、SRID、MVTとbbox上限、取込rollback対象を確認する。原本・NDJSONはGitへ追加しない。
