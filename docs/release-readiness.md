...
6. **未完了 — 実機PWA**: iPhone Safari/PWAとAndroid Chrome/PWAで、インストール、1画面の田んぼ登録、刈りどき順一覧、通信断からの復帰、屋外視認性を実機確認します。PlaywrightのPixel 7相当エミュレーションは実機ゲートの代わりにしません。
...
現行のブラウザE2Eは地図を使わない田んぼ登録、収穫記録、パスワード再設定の縦通しを対象にしています。Google OAuth、実機PWA、本番メール配送はまだ自動受入していません。MAFF候補のタップ選択とMVT表示は通常の農家向け導線から外し、旧基盤の統合テストとしてのみ維持します。
