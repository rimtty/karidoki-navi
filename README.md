...
現在の実証対象は三原市久井町です。最初から選べる品種マスターは次の5品種です。
...
この5品種について、久井町へ適用できる公的な出穂後積算気温の閾値はまだ確認できていません。品種は選択できますが、該当する公式値がない場合は画面に「公式ルール未設定」と表示し、他地域の値を推測で転用しません。利用者は一覧にない品種を自分のアカウントへ追加でき、根拠メモ付きの刈りどきの目安を登録できます。目安未設定の田んぼへ後から初めて設定した場合だけ自動反映し、すでに使っている過去の目安は書き換えません。方針の背景は [ユーザー定義品種ルールADR](docs/decisions/0003-user-defined-variety-rules.md) を参照してください。
...
| `/app/guide` | 目安設定、田んぼ登録、毎日の確認を3段階で案内する使い方画面 |
| `/app/settings/variety-rules` | 品種追加と、品種別の「刈り始め・刈り終わり」の目安を大きな文字で登録・変更・削除 |
...
`e2e:local`は先に `pnpm exec supabase start` を実行し、ローカルSupabaseだけを使います。Pixel 7相当のモバイルChromeで、未認証リダイレクト、メールログイン、使い方ガイド、品種追加、1画面の田んぼ登録、後からの目安反映、一覧・詳細、収穫登録、ログアウトに加え、ローカル受信箱の再設定メールから新パスワードで再ログインするまでを確認します。専用ユーザーとテスト結果の扱いは [モバイルE2E README](tests/e2e/README.md) を参照してください。
